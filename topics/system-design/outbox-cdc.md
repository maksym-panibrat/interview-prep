...
- **Commit then publish, publish fails.** The DB says yes; the broker times out or the process crashes between the two lines. The order is placed, no one downstream knows. Inventory never decrements; the email never sends.
- **Publish then commit, commit fails.** You moved the publish first; now the broker has an event for an order the DB rolled back. Downstream services act on a phantom.
...
The relay still couples the publish path to your app. **Change data capture** removes that by tailing the database's own write-ahead log. Postgres has logical replication slots and `pgoutput`; MySQL has the binlog; MongoDB the oplog. Debezium (inside Kafka Connect, the canonical choice) subscribes to the WAL, decodes each committed change, and emits a Kafka record per row. Because the WAL is single-writer-ordered per shard, CDC preserves commit order; because it reads only committed transactions, it never publishes phantoms from a rolled-back txn.

The full topology:

```
  app                 Postgres                  Kafka Connect              Kafka              consumers
   |  BEGIN              |                            |                       |                    |
   |  UPDATE orders ---> |                            |                       |                    |
   |  INSERT outbox ---> | (data + outbox row)        |                       |                    |
   |  COMMIT             | --- WAL record ----------> | Debezium decode       |                    |
   |                     |  (replication slot)        | + outbox SMT routing  |                    |
   |                     |                            | -- topic record ----> | OrderPlaced topic  |
   |                     |                            |                       | --- consume -----> |
```

Confluent's "outbox event router" SMT (single-message transform) reshapes raw `outbox_events` row events into per-event-type topics so consumers don't subscribe to your raw table.
...
- **Replication-slot leak (Postgres).** A logical slot that stops being consumed pins WAL forever — Postgres will not recycle beyond the oldest slot's position. The disk fills, the database goes read-only, and the on-call learns about slots the hard way. Alert on `pg_replication_slots` lag.
- **WAL retention must outlive CDC downtime.** If Debezium is offline longer than your `wal_keep_size` / archive retention, the slot's required WAL is gone and the connector cannot resume without a snapshot.
